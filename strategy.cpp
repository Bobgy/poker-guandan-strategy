...
// int ExistShunZi(THandCards& hc, int StartingNumber, int Length, int HandNum,
//                 char Suit, THandCards& oneHand) {
//     bool f = true;
//     oneHand.clear();
//     if (Suit == 'A') {
...
// int check(THandCards& hc, list<string>& ASolution, int CurrentTypePosition,
...
//             THandCards oneHand;
//             int status =
//                 ExistShunZi(hc, StartingNumber, 5, 1, NowSuit, oneHand);
//             if (status > 0) {
//                 Chu(hc, oneHand);
//                 list<string> tmpSolution;
//                 int t = check(hc, tmpSolution, TypePosition,
...
//     TypePosition++;
//     if (CurrentTypePosition <= TypePosition) {
//         if (CurrentTypePosition < TypePosition) CurrentStartingNumPosition =
//         0; for (it = hc.begin(); it != hc.end(); it++) {
//             int StartingNumber = (*it).first;
//             if (StartingNumber < CurrentStartingNumPosition) continue;
//             THandCards oneHand;
//             int status =
//                 ExistShunZi(hc, StartingNumber, 3, 2, NowSuit, oneHand);
//             if (status > 0) {
//                 Chu(hc, oneHand);
//                 list<string> tmpSolution;
...
//             THandCards oneHand;
//             int status =
//                 ExistShunZi(hc, StartingNumber, 2, 3, NowSuit, oneHand);
//             if (status > 0) {
//                 Chu(hc, oneHand);
//                 list<string> tmpSolution;
//                 int t = check(hc, tmpSolution, TypePosition, StartingNumber);
...
// int TestTrumpCard(THandCards& hc, int n, int& min, TSolutions& solution, int
// si,
//                   int sj, THandCards& UsedAs) {
//     if (n <= 0) {
...
// 数字10：0?
// // | 其余和牌面相同
// // mainRank: main rank, starting from 2
// int EMSCRIPTEN_KEEPALIVE calc() {
//     string cards = "BJADKDJH0S0C0CAH8D7S7H4H4D4H3D3C2H9C7CAH6C5CKSQSJS0S9S";
//     char mainRank = 'A';
